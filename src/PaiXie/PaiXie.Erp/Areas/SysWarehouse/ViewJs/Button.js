...
$("#btnSave").click(function () {
 ...
 //   if (idList != "") {
       // alert(idList);
                $.ajax({
                    url: "/SysWarehouse/role/SetRoleButton?rcode=" + $("#rolecode").val() + "&mids=" + idList,
                    type: "GET",
                    cache: false,
                    success: function (r) {
                        var map = $.parseJSON(r);
...
function loadtreegrid() {
    $('#test').treegrid({
        url: "/SysWarehouse/role/Gettreebutton?roleid=" + $('#rolecode').val() + "&ram=" + Math.random(),
        idField: 'id',
        treeField: 'text',
        animate: "true",
        rownumbers: false,
        ...
        columns: [[
            {
                ...
                title: '菜单事件列表', field: 'text', formatter: function (value, rowData, rowIndex) {
