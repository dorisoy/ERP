...
function initTable(queryData, pageNumber) {
    ...
    $('#grid').datagrid({
        url: '/sys/logs/hislogsearch?ram=' + Math.random(),
...
function initTable2(queryData, pageNumber) {
    ...
    $('#grid2').datagrid({
        url: '/sys/logs/syslogsearch?ram=' + Math.random(),
        height: $(document).height() * 0.8,
        width: function () { return document.body.clientWidth * 0.9 },
        nowrap: false,
        autoRowHeight: false,
...
function seeMessage(id) {
    ...
    showLocalWindow("操作内容", "/sys/logs/SeeMessage?id=" + id, 500, 300, true, false, false);
}