...
$(function () {
    ...
    $('#orderRefundType').combobox({
        url: '/Warehouse/OrderRefund/GetOrderRefundTypeJson',
        valueField: 'Value',
        textField: 'Name',
    ...
    $('#orderRefundDuty').combobox({
        url: '/Warehouse/OrderRefund/GetOrderRefundDutyJson',
        valueField: 'Value',
        textField: 'Name',
        onLoadSuccess: function () { //数据加载完毕事件
            $(this).combobox('select', "-1");
    ...
    //添加售后
    $('#add').click(function () {
        location.href = "/Warehouse/OrderRefund/Add";
    });
    //搜索
...
function initTable(queryData, pageNumber) {
    $('#grid').datagrid({
        url: '/Warehouse/OrderRefund/Search?ram=' + Math.random(),
...
function showOrderRefundDetails(billNo) {
    ...
    var src = "/Warehouse/OrderRefund/Details?billNo=" + billNo;
    //var re = $(this).attr("re");
