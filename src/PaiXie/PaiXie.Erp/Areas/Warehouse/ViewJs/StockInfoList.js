...
function initTable(queryData, pageNumber) {
    $('#grid').datagrid({
        url: '/Warehouse/Stock/Search?ram=' + Math.random(),
        height: '100%',
        width: '100%',
        nowrap: false,
        autoRowHeight: false,
        ...
        columns: [[
			...
			{ title: '商品编码', field: 'Code', width: 80, align: 'center', sortable: false },
			{
			    ...
			    formatter: function (value, row) {
			        return '<a href="#" onclick=\'show(\"' + row.Code + '\")\'>' + value + '</a>';
			    },
			    sortable: false
			},
...
function show(productsCode) {
    location.href = "/Warehouse/StockItem/Index?productsCode=" + productsCode + "&startDate=" + $("#txtStartDate").combobox('getValue') + "&endDate=" + $("#txtEndDate").combobox('getValue');
}
