...
$(function () {
    ...
    $("#returnProduct").click(function () {
        showLocalWindow("退回商品", "/Warehouse/OrderRefund/SetLogistics?ordRefundID=" + $("#hdnOrdRefundID").val(), 520, 320, true, false, false);
    });
    //确认收货
    $("#confirmReceive").click(function () {
        showLocalWindow("确认收货", "/Warehouse/OrderRefund/ConfirmReceiveIndex?ordRefundID=" + $("#hdnOrdRefundID").val(), 820, 530, true, false, false);
    });
    //取消售后
...
function initOrdRefundInfo() {
    ...
    $.ajax({
        url: "/Warehouse/OrderRefund/GetOrdRefundInfo?billNo=" + billNo,
        type: "GET",
        cache: false,
        success: function (r) {
            var map = $.parseJSON(r);
...
function cancel() {
    $.messager.confirm('提示', "确认取消该售后单吗？", function (r) {
        if (r) {
            var billNo = $("#hdnBillNo").val();
            $.ajax({
                url: "/Warehouse/OrderRefund/Cancel?billNo=" + billNo,
                type: "GET",
                cache: false,
                success: function (r) {
                    var map = $.parseJSON(r);
