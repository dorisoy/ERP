...
$(function () {
    ...
    //添加
    $("#add").click(function () {
        showLocalWindow("添加结构", "/Warehouse/AreaStruct/Edit?id=0&parentID=0", 450, 270, true, false, false);
    ...
    $("#del").click(function () {
        ...
        if (ids.length > 0) {
            $.messager.confirm('提示', "确认删除所选结构？", function (r) {
                if (r) {
                    $.ajax({
                        url: "/Warehouse/AreaStruct/Delete?ids=" + ids.join(','),
                        type: "GET",
                        cache: false,
                        success: function (r) {
                            var map = $.parseJSON(r);
...
function initTable(pageNumber) {
    $('#grid').treegrid({
        url: '/Warehouse/AreaStruct/Search?ram=' + Math.random(),
        height: '100%',
        width: '100%',
        idField: 'id',
        treeField: 'text',
        ...
        columns: [[
            ...
            {
                title: '结构名称', field: 'text', formatter: function (value, rowData, rowIndex) {
            ...
            },
            {
                ...
                formatter: function (value, rowData) {
                    var addSubStr = rowData.attributes.attr > 0 ? '<a href="javascript:void(0);" style="color:#ccc;">添加下级<a>' : '<a href="javascript:void(0);" onclick=\'addSubAreaStruct(' + rowData.id + ')\'>添加下级</a>';
                    return addSubStr + '&nbsp;|&nbsp;<a href="#" onclick=\'editAreaStruct(' + rowData.id + ')\'>编辑</a>';
                },
                sortable: false
            }
        ]],
...
function editAreaStruct(id) {
    showLocalWindow("编辑结构", "/Warehouse/AreaStruct/Edit?id=" + id + "&parentID=0", 450, 270, true, false, false);
}

function addSubAreaStruct(parentID) {
    showLocalWindow("添加下级", "/Warehouse/AreaStruct/Edit?id=0&parentID=" + parentID, 450, 270, true, false, false);
}
