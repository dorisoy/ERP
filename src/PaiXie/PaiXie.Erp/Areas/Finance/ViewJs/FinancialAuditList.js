...
function initTable(queryData, pageNumber) {
    ...
    {
        $("#" + gridID).datagrid({
            url: '/Finance/FinancialAudit/Searchpd?ram=' + Math.random(),
            height: '100%',
            width: '100%',
            nowrap: false,
            autoRowHeight: false,
            ...
            columns: [[
               ...
               { title: '', field: 'ID', hidden: true },
                ...
                { title: '盘点仓库', field: 'WarehouseName', width: 150, align: 'center', sortable: true },
              ...
              { title: '盘点库区', field: 'LocationName', width: 150, align: 'center', sortable: true },
                ...
                { title: '操作人员', field: 'CreatePerson', width: 100, align: 'center', sortable: true },
                {
                ...
                },
                {
                    ...
                    formatter: function (value, row) {
                        var html = '';
                        if (row.Status == 10) {
                            html = "<a href=\"javascript:void(0);\" onclick=\"Gotopddetail('" + row.ID + "');\">核对明细</a>";
                            html += ' <br/> <a href="javascript:void(0);" onclick=\'PDshenhe(' + row.ID + ');\' >审核完毕</a>';
                       ...
                       { title: '状态', field: 'StatusName', width: 80, align: 'center', sortable: true },
            {
                ...
                formatter: function (value, row) {
                    ...
                    if (row.IsAuditPrice == 0) {
                        html = "<a href=\"javascript:void(0);\" onclick=\"Gotodetail('" + row.ID + "','" + row.BillType + "','" + row.BillNo + "');\">核对入库价</a>";
...
//盘点审核
function PDshenhe(id) {
    $.messager.confirm('提示', "确认对该单据进行审核？", function (r) {
        if (r) {
            $.ajax({
                url: "/Finance/FinancialAudit/PDshenhe?ram=" + Math.random() + "&id=" + id,
                type: "GET",
                cache: false,
                success: function (r) {
                    var map = $.parseJSON(r);
...
//核对入库价
function Gotodetail(id, type, BillNo) {
    if (type == 10 || type == 30) //入库明细
    {
     //   alert(id);
       // alert(BillNo);
      location.href = "/Finance/FinancialAudit/InStock?ram=" + Math.random() + "&BillNo=" + BillNo + "&outInStockID=" + id;
    }
    else  //出库明细
    {
...
//盘点详情 
function Gotopddetail(id) {
   
        location.href = "/Finance/FinancialAudit/pd?ram=" + Math.random() + "&id=" + id;
